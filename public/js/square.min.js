...
$(document).ready(function(){$("body").append('<div id="backtotop"><a href="#"></a></div>');var c=100;var d=500;$(window).scroll(function(){if($(window).scrollTop()>=c){$("#backtotop").addClass("visible uk-animation-slide-bottom")}else{$("#backtotop").removeClass("visible uk-animation-slide-bottom")}});$("#backtotop a").on("click",function(){$("html, body").animate({scrollTop:0},d);return false});$("#uploadFile").on("change",function(e){var name=e.currentTarget.files[0].name;$("#uploadFileName").text(name)});$("#uploadAvator").on("change",function(e){var name=e.currentTarget.files[0].name;$("#uploadAvatorName").text(name);var path=getobjecturl(e.currentTarget.files[0]);$("#my_avator").attr("src",path)});$("#userBackfile").on("change",function(e){var name=e.currentTarget.files[0].name;var path=getobjecturl(e.currentTarget.files[0]);$("#userBackreal").attr("src",path)});toastr.options.positionClass="toast-top-center";$(".fastcomment").keypress(function(e){if(e.which===13){fastcomment($(this).attr("data-id"),$(this).val())}})});function getobjecturl(file){var url=null;if(window.createObjectURL!=undefined){url=window.createObjectURL(file)}else{if(window.URL!=undefined){url=window.URL.createObjectURL(file)}else{if(window.webkitURL!=undefined){url=window.webkitURL.createObjectURL(file)}}}return url};
